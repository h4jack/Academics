...
void Simulator::sender(int from)
{
    ...
    for (int i = from; i < from + windowSize && i < totalFrames; i++)
    {
        if (i == faultIndex && faultHappens == 0)
            continue;
        cout << "   sent frame " << frames[i].data << " SEQ: " << frames[i].seq << endl;
...
void Simulator::recieveACK()
{
    ...
    while (!ACKS.empty())
    {
        cout << "   Recieved ACK for frame SEQ: " << ACKS.front() << endl;
        i++;
        ACKS.pop();
    }
    if (faultHappens == 0)
    {
        cout << "SENDER:\n    Haven't Recieved ack for frame: " << frames[faultIndex].seq << endl;
        cout << "\n    Resending frames... from SEQ: " << frames[faultIndex].seq << endl;
        sender(faultIndex);
        reciever();
        recieveACK();
    }
